...
namespace velodyne_pointcloud 
{
  ...
  void Calibration::read(const std::string& calibration_file) {
    ...
    std::cout<<"read yaml file: "<<calibration_file<<std::endl;
    if (!fin.is_open()) {
      std::cout<<"fin.is_open()=false."<<std::endl;
      initialized = false;
      return;
    }
    std::cout<<"fin.is_open()=true."<<std::endl;
    initialized = true;
    try {
      YAML::Node doc;
#ifdef HAVE_NEW_YAMLCPP
      ...
      std::cout<<"yml file loaded."<<std::endl;
#else
      YAML::Parser parser(fin);
      parser.GetNextDocument(doc);
      std::cout<<"yml file parser."<<std::endl;
#endif
      doc >> *this;
